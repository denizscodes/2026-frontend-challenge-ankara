  ...
  if (error) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center p-8 text-center">
        <h2 className="text-2xl font-bold text-gray-900">Investigation Offline</h2>
      ...
      <header className="mb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-10">
          <Card className="bg-white border-primary/20 shadow-sm relative overflow-hidden group">
            ...
            <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Total Raw Leads</p>
            <h3 className="text-3xl font-black text-gray-900 mt-1">{loading ? '...' : totalLeads}</h3>
          ...
          <Card className="bg-white border-primary/20 shadow-sm relative overflow-hidden group">
            ...
            <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Unique Identities</p>
            <h3 className="text-3xl font-black text-gray-900 mt-1">{loading ? '...' : linkedPeople.length}</h3>
          ...
          <Card className="bg-dark text-white border-none shadow-xl relative overflow-hidden group">
            ...
            <p className="text-xs font-bold text-gray-400 uppercase tracking-wider relative z-10">Correlation Hits</p>
              ...
              <div className="grid grid-cols-1 gap-3">
                {filteredPeople.map((person) => (
                  ...
                  >
                    ...
                    <h3 className="font-bold text-gray-900 group-hover:text-primary transition-colors">{person.name}</h3>
                    ...
                    <div className="mt-3 pt-3 border-t border-gray-50 flex items-center justify-between">
                       <span className="text-[9px] font-mono text-gray-400 bg-gray-50 px-1.5 py-0.5 rounded uppercase">ID: {person.id.slice(0, 8)}...</span>
                       <ChevronRight className={`h-4 w-4 transition-transform ${selectedPerson?.id === person.id ? 'text-primary translate-x-1' : 'text-gray-300'}`} />
                    </div>
                  </button>
                ))}
              </div>
        ...
        <div className="flex-grow">
          <AnimatePresence mode="wait">
            {selectedPerson ? (
              ...
              >
                ...
                <Card className="p-8 border-none shadow-2xl relative overflow-hidden bg-white">
                  ...
                  <div className="relative z-10 flex flex-col md:flex-row gap-8 items-start">
                    <div className="w-24 h-24 rounded-3xl bg-gray-100 flex items-center justify-center text-gray-300 shrink-0 border-4 border-white shadow-xl">
                      <Users className="h-12 w-12" />
                    </div>
                    <div className="flex-grow">
                       <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className="text-[10px] font-bold text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">REL_INDEX: {selectedPerson.reliability.toFixed(1)}%</span>
                       </div>
                       <h2 className="text-4xl font-black text-gray-900 leading-tight">{selectedPerson.name}</h2>
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4 mt-6">
                          <div className="flex items-center gap-3 text-sm text-gray-600">
                             <div className="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center text-gray-400"><Mail className="h-4 w-4" /></div>
                             <div>
                          ...
                          <div className="flex items-center gap-3 text-sm text-gray-600">
                             <div className="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center text-gray-400"><Phone className="h-4 w-4" /></div>
                             <div>
                          ...
                          <div className="flex items-center gap-3 text-sm text-gray-600">
                             <div className="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center text-gray-400"><MapPin className="h-4 w-4" /></div>
                             <div>
                                <p className="text-[10px] font-bold text-gray-400 uppercase">Primary Location</p>
                                <p className="font-semibold">{selectedPerson.location || 'Unknown'}</p>
                             </div>
                          </div>
                          <div className="flex items-center gap-3 text-sm text-gray-600">
                             <div className="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center text-gray-400"><Database className="h-4 w-4" /></div>
                             <div>
                                <p className="text-[10px] font-bold text-gray-400 uppercase">Active Threads</p>
                  ...
                  <div className="relative space-y-6 before:absolute before:left-[19px] before:top-4 before:bottom-4 before:w-0.5 before:bg-gray-100">
                    {selectedPerson.submissions.map((sub, idx) => (
                      <div key={sub.id} className="relative pl-12">
                        <div className="absolute left-0 top-1.5 w-10 h-10 rounded-full bg-white border-2 border-primary flex items-center justify-center z-10 shadow-sm">
                           ...
                           <div className="flex items-start justify-between mb-4">
                              <div>
                                 <div className="flex items-center gap-2 mb-1">
                                    ...
                                    <span className="text-[10px] text-gray-400 font-mono">#{sub.id.slice(-8)}</span>
                                 </div>
                                 <h4 className="font-bold text-gray-900">Lead Contribution</h4>
                              </div>
                              <div className="text-right">
                                 ...
                                 <p className="text-[10px] text-gray-400 font-medium">{new Date(sub.created_at).toLocaleTimeString()}</p>
                              </div>
                           </div>
                           <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50/50 p-4 rounded-xl border border-gray-100">
                              {Object.values(sub.answers).slice(0, 4).map((ans: any, i) => (
                                 <div key={i}>
                                    ...
                                    <p className="text-sm text-gray-700 mt-0.5 font-medium line-clamp-2">{ans.answer || '—'}</p>
                                 </div>
                              ))}
                           </div>
              ...
              <div className="h-full flex flex-col items-center justify-center p-20 border-2 border-dashed border-border rounded-3xl bg-gray-50/30">
                <div className="w-20 h-20 rounded-full bg-gray-50 flex items-center justify-center text-gray-300 mb-6">
                   <Fingerprint className="h-10 w-10" />
                </div>
                <h3 className="text-2xl font-bold text-gray-900">Select an Identity</h3>
                <p className="text-gray-500 mt-2 max-w-sm text-center font-medium">
